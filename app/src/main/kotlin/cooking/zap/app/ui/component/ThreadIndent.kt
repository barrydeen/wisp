 ...
 * never runs out of horizontal space (the failure mode that flattened deep threads before).
 */
fun threadIndentDp(depth: Int, cap: Int = ThreadFlattener.DEPTH_CAP, step: Dp = INDENT_STEP): Dp =
    step * min(depth, cap)

/**
 * iOS-parity depth connector: a single vertical rail into a rounded corner and a short
 * horizontal run to the reply — instead of one straight line per ancestor level. Drawn
 * behind a row when [show] is true (depth > 0). [indent] is the row's start padding (from
 * [threadIndentDp]); the rail lands one corner radius left of the card's padded edge so the
 * arc's horizontal run meets the card edge exactly.
 *
 * When [dashedTop] is true the top of the rail is drawn dashed, signalling that the guide
 * line continues upward to a parent that isn't the row directly above (e.g. the first reply
 * of a branch, or a reply revealed by expanding a folded subtree) — so the rail doesn't look
 * like it starts "in mid-air."
 *
