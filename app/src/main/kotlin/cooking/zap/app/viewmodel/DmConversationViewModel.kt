...
class DmConversationViewModel(app: Application) : AndroidViewModel(app) {
                    ...
                    try {
                        ...
                        if (Nip17.isReaction(rumor)) {
                            ...
                            val emojiUrl = if (emojiContent.startsWith(":") && emojiContent.endsWith(":")) {
                                cooking.zap.app.nostr.Nip30.parseEmojiTags(rumor.tags)[emojiContent.removeSurrounding(":")]
                            } else null
                        ...
                        val emojiMap = cooking.zap.app.nostr.Nip30.parseEmojiTags(rumor.tags)
                        val fileMetadata = if (Nip17.isFileMessage(rumor)) {
                            cooking.zap.app.nostr.EncryptedMedia.parseKind15Tags(rumor.tags, rumor.content)
                        } else null
                        val msg = DmMessage(
    ...
    private suspend fun fetchRecipientDmRelays(relayPool: RelayPool, forceRefresh: Boolean = false): List<String> {
        ...
        return cooking.zap.app.repo.DmRelayLookup.fetch(peerPubkey, relayPool, repo, forceRefresh)
    }
    ...
    fun sendMessage(relayPool: RelayPool, signer: NostrSigner? = null, resolvedEmojis: Map<String, String> = emptyMap()) {
        ...
        if (signer != null) {
            ...
            viewModelScope.launch(Dispatchers.Default) {
                try {
                    ...
                    val emojiTags = cooking.zap.app.nostr.Nip30.buildEmojiTagsForContent(text, resolvedEmojis)
                    ...
                    val sentEmojiMap = cooking.zap.app.nostr.Nip30.buildEmojiTagsForContent(text, resolvedEmojis)
        ...
        viewModelScope.launch(Dispatchers.Default) {
            try {
                val senderPubkeyHex = keypair.pubkey.toHex()
                val emojiTags2 = cooking.zap.app.nostr.Nip30.buildEmojiTagsForContent(text, resolvedEmojis)
