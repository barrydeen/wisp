...
/**
 * The Antz app logo — a glowing core with flowing antz trails.
 *
 * @param size The overall size of the logo mark (icon only)
 * @param showText Whether to show the "antz" wordmark below the icon
 */
@Composable
fun AntzLogo(
    ...
    Column(
        ...
        Canvas(modifier = Modifier.size(size)) {
            drawAntzMark(primary, secondary)
        }
...
private fun DrawScope.drawAntzMark(primary: Color, secondary: Color) {
    val w = size.width
    val h = size.height

    // Center-bottom is the origin point of the antz
    val coreX = w * 0.5f
    val coreY = h * 0.72f

    // --- Main antz trail (center, flowing upward with an elegant S-curve) ---
    ...
    // --- Left antz trail (shorter, curves left) ---
    ...
    // --- Right antz trail (shortest, quick flick right) ---
