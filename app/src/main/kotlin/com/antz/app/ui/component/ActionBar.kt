...
fun ActionBar(
    ...
    val prefs = remember { context.getSharedPreferences("antz_settings", android.content.Context.MODE_PRIVATE) }
    ...
    Row(modifier = modifier, verticalAlignment = Alignment.CenterVertically) {
        ...
        Text(
            ...
            style = MaterialTheme.typography.labelSmall,
            color = if (userReactionEmojis.isNotEmpty()) AntzThemeColors.zapColor else MaterialTheme.colorScheme.onSurfaceVariant
        )
        Spacer(Modifier.width(8.dp))
        Box {
            IconButton(onClick = { showRepostMenu = true }) {
                Icon(
                    ...
                    tint = if (hasUserReposted) AntzThemeColors.repostColor else MaterialTheme.colorScheme.onSurfaceVariant,
                    modifier = Modifier.size(22.dp)
                )
            }
            if (showRepostMenu) {
        ...
        Text(
            ...
            style = MaterialTheme.typography.labelSmall,
            color = if (hasUserReposted) AntzThemeColors.repostColor else MaterialTheme.colorScheme.onSurfaceVariant
        )
        Spacer(Modifier.width(8.dp))
        Box {
            IconButton(onClick = onZap, enabled = !isZapInProgress) {
                if (isZapInProgress) {
                    ...
                    Icon(
                        ...
                        tint = if (hasUserZapped) AntzThemeColors.zapColor else MaterialTheme.colorScheme.onSurfaceVariant,
        ...
        if (!isZapInProgress && zapSats > 0) {
            Text(
                ...
                style = MaterialTheme.typography.labelSmall,
                color = if (hasUserZapped) AntzThemeColors.zapColor else MaterialTheme.colorScheme.onSurfaceVariant
            )
        }
        Spacer(Modifier.width(8.dp))
        IconButton(onClick = onAddToList) {
            Icon(
                ...
                tint = if (isInList) AntzThemeColors.bookmarkColor else MaterialTheme.colorScheme.onSurfaceVariant,
                modifier = Modifier.size(22.dp)
            )
        }
    }
...
internal fun LightningAnimation(modifier: Modifier = Modifier) {
    ...
    val paidColor = AntzThemeColors.paidColor
    val zapColor = AntzThemeColors.zapColor
