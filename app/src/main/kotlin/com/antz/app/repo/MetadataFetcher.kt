...
class MetadataFetcher(
    ...
    fun requestPollVotes(pollEventId: String) {
        ...
        val filter = Filter(kinds = listOf(com.antz.app.nostr.Nip88.KIND_POLL_RESPONSE), eTags = listOf(pollEventId))
        ...
        if (pollEvent != null) {
            ...
            for (url in com.antz.app.nostr.Nip88.parsePollRelays(pollEvent)) {
    ...
    fun requestZapPollVotes(pollEventId: String) {
        ...
        if (pollEvent != null) {
            val sentUrls = relayPool.getReadRelayUrls().toSet() + relayPool.getWriteRelayUrls().toSet()
            for (url in com.antz.app.nostr.Nip69.parseZapPollRelays(pollEvent)) {
                if (url !in sentUrls) relayPool.sendToRelayOrEphemeral(url, msg)
            }
        }
        scope.launch {
    ...
    fun fetchQuotedEvents(event: com.antz.app.nostr.NostrEvent) {
