...
import com.antz.app.nostr.Nip13
...
import com.antz.app.nostr.SignerCancelledException
import com.antz.app.repo.MuteRepository
import com.antz.app.nostr.hexToByteArray
import com.antz.app.nostr.toHex
import com.antz.app.relay.RelayConfig
import com.antz.app.relay.RelayEvent
...
class DmConversationViewModel(app: Application) : AndroidViewModel(app) {
                    ...
                    try {
                        ...
                        if (Nip17.isReaction(rumor)) {
                            ...
                            val emojiUrl = if (emojiContent.startsWith(":") && emojiContent.endsWith(":")) {
                                com.antz.app.nostr.Nip30.parseEmojiTags(rumor.tags)[emojiContent.removeSurrounding(":")]
                            } else null
                        ...
                        val emojiMap = com.antz.app.nostr.Nip30.parseEmojiTags(rumor.tags)
                        val fileMetadata = if (Nip17.isFileMessage(rumor)) {
                            com.antz.app.nostr.EncryptedMedia.parseKind15Tags(rumor.tags, rumor.content)
                        } else null
                        val msg = DmMessage(
    ...
    fun sendMessage(relayPool: RelayPool, signer: NostrSigner? = null, resolvedEmojis: Map<String, String> = emptyMap()) {
        ...
        if (signer != null) {
            ...
            viewModelScope.launch(Dispatchers.Default) {
                try {
                    ...
                    val emojiTags = com.antz.app.nostr.Nip30.buildEmojiTagsForContent(text, resolvedEmojis)
                    ...
                    val sentEmojiMap = com.antz.app.nostr.Nip30.buildEmojiTagsForContent(text, resolvedEmojis)
        ...
        viewModelScope.launch(Dispatchers.Default) {
            try {
                val senderPubkeyHex = keypair.pubkey.toHex()
                val emojiTags2 = com.antz.app.nostr.Nip30.buildEmojiTagsForContent(text, resolvedEmojis)
