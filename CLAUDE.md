...
- **Signer abstraction** — `NostrSigner` is an interface with two implementations: `LocalSigner` (key held on-device) and `RemoteSigner` (NIP-55 external signing via Amber — silent ContentResolver path first, intent approval fallback through `SignerIntentBridge`). There is **no NIP-46 bunker signer**. `SigningMode` is `LOCAL`/`REMOTE`/`READ_ONLY`; `READ_ONLY` accounts hold no key and cannot sign — gate signing-dependent features (e.g. NIP-98 auth) behind "account has a signing key."
